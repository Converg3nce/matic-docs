...
Secondly, Plasma Cash, which is what we believe Loom wants to use "in future", block times, will always be more than the Ethereum block times as you need to push every block of the sidechain to the main chain, while Matic uses checkpoints (with PoS layer) for 1-second block times (with DPoS layer)
...
[Opensigner](https://github.com/OpenSigner/opensigner)
